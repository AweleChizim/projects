...
const RecordView: React.FC<Props> = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch(`/api/records/view/${recordIdInput.current!.value}`);
    ...
    return (
        <div className='flex flex-col space-y-3'>
            <div className='text-green-800 bg-green-300 font-bold w-full'>View Patients Record Information</div>
            <p>
                <input 
                    ref={recordIdInput}
                    type="text"
                    placeholder="recordId (1 to 10) here"
                    defaultValue={1}
                />
            </p>
            <p>
