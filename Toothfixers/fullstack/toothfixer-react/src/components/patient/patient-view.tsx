...
const PatientView: React.FC<Props> = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch(`/api/patients/view/${patientIdInput.current!.value}`);
    ...
    return (
        <div className='flex flex-col space-y-3'>
            <div className='text-green-800 bg-green-300 font-bold w-full'>View Patient Information</div>
            <p>
                <input 
                    ref={patientIdInput}
                    type="text"
